...
3. **配置 Workers 路由**
   ...
   - 配置：
     ...
     - **路由 (Route)**: `api.kinkosyouji.co.jp/*`
     - **失败模式**: 选择 "失败时自动关闭 (阻止)"
   - 点击 **添加路由**
...
5. **测试**
   ```bash
   curl https://api.kinkosyouji.co.jp/api/products
   ```

## 重要提示

⚠️ **在添加路由时，必须确保：**
- **区域 (Zone)** 选择的是 `kinkosyouji.co.jp`（不是其他域名）
- **路由 (Route)** 输入的是 `api.kinkosyouji.co.jp/*`

如果 Zone 下拉菜单中没有 `kinkosyouji.co.jp`，说明该域名还没有添加到 Cloudflare，需要先添加域名。
...
1. **管理前端环境变量** (在 Vercel Dashboard 中)
   - `NEXT_PUBLIC_API_URL`: `https://api.kinkosyouji.co.jp`
...
3. **提交更改**
   ...
   git commit -m "更新 API URL 为 api.kinkosyouji.co.jp"
   git push
   ```
...
- 检查 DNS 记录是否正确
- 等待 DNS 传播（最多 24 小时）
- 使用 `dig api.kinkosyouji.co.jp` 检查

### 路由未生效
- 确认 Workers 路由配置正确
- **确认 Zone 选择的是 `kinkosyouji.co.jp`**
...
### 域名不在 Cloudflare 中
如果 `kinkosyouji.co.jp` 不在你的 Cloudflare 账户中：
1. 在 Cloudflare Dashboard 首页点击 "添加站点"
2. 输入 `kinkosyouji.co.jp`
3. 按照提示完成 DNS 配置
4. 更新域名服务器（如果需要）
5. 然后再配置 Workers 路由
